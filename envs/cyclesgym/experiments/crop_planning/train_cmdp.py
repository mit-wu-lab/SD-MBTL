...
if __name__ == '__main__':
    ...
    if args['fixed_weather'] == 'True':
        ...
        if args['multi']:
            ...
            weather_generator_kwargs = {
                'base_weather_file': CYCLES_PATH.joinpath('input', f"Toy_temp{args['temperature']}_sun{args['sunlight']}_prec{args['precipitation']}.weather")}
    else:
    ...
    config = dict(train_start_year=train_start_year, train_end_year=train_end_year, eval_start_year=eval_start_year, eval_end_year=eval_end_year,
                  total_timesteps=20000, eval_freq=1000, n_steps=80, batch_size=80, n_epochs=10, run_id=0,
    ...
    wandb.init(
        ...
        save_code=True,
        dir=PROJECT_PATH.joinpath(f"results/tempmulti_sunmulti_precmulti_trial{args['seed']}") if args['multi'] else PROJECT_PATH.joinpath(f"results/temp{args['temperature']}_sun{args['sunlight']}_prec{args['precipitation']}_trial{args['seed']}"),
    )
